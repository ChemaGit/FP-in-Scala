...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  */
  ...
  large lists. With lists, it's common to use a temporary, mutable buffer internal to the
  function (with lazy lists or streams, we don't normally do this). So long as the
  ...
  doesn't require even local mutation. We'll write a reverse function.
  */
...
object Main extends App {
  ...
  println(s"the result of the following expression will b: $x")
  ...
  println(s"init - List.init(List(1,2,3,4)): ${List.init(List(1,2,3,4))}")
